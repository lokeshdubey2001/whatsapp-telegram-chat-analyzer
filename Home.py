...
with open('add_ons/styles/prod_sliding_photos.html', encoding='utf-8') as f:
    prod_sliding_photos = f.read()

def load_test_data():
    progress_bar = st.progress(0, text="Loading...")
    # data = requests.get(TEST_DATA_URL).text
    with open(CHAT_EXAMPLE_PATH, encoding='utf-8') as f:
        data = f.read()
...
def load_data(files):
    ...
    for index, file in enumerate(files):
        if file.name.endswith('.txt'):
            df_list.append(_df_from_str(file.read().decode('utf-8')))
            st.session_state['file_name'] = file.name.replace('.txt', '')\
                .replace('WhatsApp Chat with', '').replace('_','')

        elif file.name.endswith('.html'):
            try:
                group_name, group_df = parse_telegram_html(file.read().decode('utf-8'))
...
def main():
    ...
    title = {'en': "Chat Analyzer"}
    st.title(title[language])

    home_holder = st.empty()
    home_holder_text = {"en": "Supports Whatsapp (.txt) / Telegram (.html)"}
    ...
    upload_text = {"en": "Choose a TXT / HTML file/s"}
    ...
    example_text = {"en": "Load Chat Example File"}
    ...
    with how_to_text_holder:
        whatsapp_text = {"en": "How to export WhatsApp chat "}
        telegram_text = {"en": "How to export Telegram chat "}
        clickhere_lang_dict = {"en": "Click Here"}
