...
def assign_metrics(col, totals_df, image, user_info, language, add_seperator=True, pct=True):
    ...
    sign = {'en': '%' if pct else 'N'}

    n_mes_lang_dict = {'en': f"{sign[language]} Messages"}
    n_wor_lang_dict = {'en': f"{sign[language]} Words"}
    n_days_lang_dict = {'en': f"{sign[language]} Active Days"}
    n_conv_lang_dict = {'en': f"{sign[language]} Conversation"}
    n_media_lang_dict = {'en': f"{sign[language]} Media"}
    emoji_lang_dict = {'en': "Top Emoji"}
...
def main():
    ...
    if 'data' not in st.session_state:
        ...
        header_text = {'en': 'User Level Analysis'}
        st.subheader(header_text[language])
        top_col, _ = st.columns((1000, 0.1))
        with top_col:
            pct_lang_dict = {'en': "Show Percentages"}
            ...
            emohi_picker_lang_dict = {'en': "Top Emoji Method"}
