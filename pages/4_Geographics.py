...
def map_query(longitude: float, latitude: float) -> dict:
    ...
    try:
        ...
        if not response.text:
            return {}  # Return an empty dictionary if response content is empty

        data = json.loads(response.text)  # Decode JSON data
        return data
...
def main():
    ...
    if 'data' not in st.session_state:
        ...
        if not locations_df.empty:
            ...
            loc_lang_dict = {'en': "Overall Locations"}
            users_lang_dict = {'en': "Overall Users with Locations"}
            ...
            with general_col:
                road_city_lang_dict = {'en': 'Top Cities & Roads'}
            ...
            no_loc_lang_dict = {"en": 'No Locations to show'}
